...
export function Reveal({ children, d = 0, y = 30 }: { children: React.ReactNode; d?: number; y?: number }) {
  const [ref, vis] = useAnim();
  return (
    <div ref={ref as any} style={{ opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : `translateY(${y}px)`, transition: `opacity 0.8s ease ${d}s, transform 0.8s ease ${d}s` }}>
      {children}
    </div>
  );
}
